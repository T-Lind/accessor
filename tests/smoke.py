...
class SmokeTests(unittest.TestCase):
    ...
    def test_spoken_agent_switch_interrupts_existing_work(self):
        ...
        app.send("29 hold")
        app.expect("▸ hold")
        app.send("switch agent to mock")
        app.expect("Interrupting current work")
        app.expect("Saved agent: mock")
        app.send("hello after switching")
        app.expect("Mock agent received: hello after switching")
        app.expect("| idle")
    ...
    def test_settings_model_selection_and_spoken_switch(self):
        ...
        app.expect("| idle")
        app.send("switch model to Sol")
        app.expect("Saved model: fixture-sol")
        app.send("which model")
        app.expect("model=fixture-sol")
        app.expect("| idle")
        app.send("switch model to imaginary")
        app.expect("not in the available list")
        app.send("/settings model default")
        app.expect("Saved model: default")

    def test_settings_wake_policy_applies_immediately(self):
        app=self.app("--agent","mock")
        app.send("/settings addressed true")
        app.expect("Saved addressed: true")
        app.send("29 hello")
        ...
        app.expect("Saved addressed: false")
    ...
    def test_addressed_mode_and_mute(self):
        app = self.app("--agent", "mock", "--addressed")
        app.send("29")
        app.expect("GREEN:")
    ...
    def test_busy_time_does_not_expire_conversation(self):
        ...
        app.expect("interrupted")
        app.expect("| idle")
        app.send("follow up after long work")
    ...
    def test_hey_wake_and_go_back_to_sleep(self):
        ...
        app.expect("| idle")
        app.send("please go back to sleep")
        app.expect("Asleep. Waiting for wake code.")
        app.send("/status")
        app.expect("WHITE:")
