...
class SmokeTests(unittest.TestCase):
    ...
    def test_barge_in_can_be_disabled(self):
        ...
        ignored = app.expect("Barge-ins are off")
        self.assertIn("followup must wait", ignored)
    ...
    def test_dashboard_setup_and_transcription_controls(self):
        app = self.app("--agent", "mock")
        app.send("/setup")
        app.expect("Setup 1/5")
        app.send("42")
        app.expect("Setup 2/5")
        app.send("9000")
        app.expect("0–3600")
        app.send("0")
        app.expect("Setup 3/5")
        app.send("system")
        app.expect("Setup 4/5")
        app.send("no")
        app.expect("Setup 5/5")
        app.send("no")
